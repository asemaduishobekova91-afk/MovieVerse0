...
from database.schema import ProductInputSchema,ProductOutSchema
...
async def create_product(product: ProductInputSchema, db: Session = Depends(det_db)):
...
async def list_product(db: Session = Depends(det_db)):
    return db.query(Product).all()

@product_router.get('/{product_id}',response_model=ProductOutSchema)
async def detail_product(product_id: int ,db: Session = Depends(det_db)):
...
async def delete_product(product_id: int,db: Session = Depends(det_db)):
    product_db = db.query(Product).filter(Product.id == product_id).first()
    if not product_db:
        raise HTTPException(detail="Мындай маалымат жок", status_code=400)
