...
from database.schema import UserProfileInputSchema,UserProfileOutSchema
...
async def create_user(user: UserProfileInputSchema, db: Session = Depends(det_db)):
...
async def list_user(db: Session = Depends(det_db)):
    return db.query(UserProfile).all()

@user_router.get('/{user_id}',response_model=UserProfileOutSchema)
async def detail_user(user_id: int ,db: Session = Depends(det_db)):
...
async def delete_user(user_id: int,db: Session = Depends(det_db)):
    user_db = db.query(UserProfile).filter(UserProfile.id == user_id).first()
    if not user_db:
        raise HTTPException(detail="Мындай маалымат жок", status_code=400)
