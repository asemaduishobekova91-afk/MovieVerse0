...
from database.schema import ReviewInputSchema,ReviewOutSchema
...
async def create_review(review: ReviewInputSchema, db: Session = Depends(det_db)):
...
async def list_review(db: Session = Depends(det_db)):
    return db.query(Review).all()

@review_router.get('/{review_id}',response_model=ReviewOutSchema)
async def detail_review(review_id: int ,db: Session = Depends(det_db)):
...
async def delete_review(review_id: int,db: Session = Depends(det_db)):
    review_db = db.query(Review).filter(Review.id == review_id).first()
    if not review_db:
        raise HTTPException(detail="Мындай маалымат жок", status_code=400)
