...
from database.schema import CategoryInputSchema,CategoryOutSchema
...
async def create_category(category: CategoryInputSchema, db: Session = Depends(det_db)):
...
async def list_category(db: Session = Depends(det_db)):
    return db.query(Category).all()

@category_router.get('/{category_id}',response_model=CategoryOutSchema)
async def detail_category(category_id: int ,db: Session = Depends(det_db)):
...
async def delete_category(category_id: int,db: Session = Depends(det_db)):
    category_db = db.query(Category).filter(Category.id == category_id).first()
    if not category_db:
        raise HTTPException(detail="Мындай маалымат жок", status_code=400)
