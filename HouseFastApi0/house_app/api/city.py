...
from house_app.database.models import City
from house_app.database.schema import CityInputSchema, CityOutSchema
...
async def update_city(city_id: int, city: CityInputSchema, db: Session = Depends(get_db)):
    ...
    for city_key, city_value in city.model_dump().items():
        setattr(city_db, city_key, city_value)

    db.commit()
    db.refresh(city_db)
    return {'message': 'Шаар ийгиликтүү жаңыртылды'}
...
async def delete_city(city_id: int, db: Session = Depends(get_db)):
    ...
    db.delete(city_db)
    db.commit()
    return {'message': 'Шаар ийгиликтүү өчүрүлдү'}
