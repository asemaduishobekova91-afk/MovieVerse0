...
from site_app.database.models import Follow
from site_app.database.schema import FollowInputSchema, FollowOutSchema
...
async def create_follow(follow: FollowInputSchema, db: Session = Depends(get_db)):
...
async def list_follow(db: Session = Depends(get_db)):
